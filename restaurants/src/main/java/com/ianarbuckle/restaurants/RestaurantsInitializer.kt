 ...
 *
 */
data class RestaurantsInitializer(val baseUrl: String, val okHttpClient: OkHttpClient, val converterFactory: Converter.Factory) {

    fun init() {
        val injector: RestaurantsInjector = DefaultRestaurantsInjector(okHttpClient, baseUrl, converterFactory)
    ...
    class Builder {
        ...
        fun build() {
            val initializer = RestaurantsInitializer(baseUrl, okHttpClient, converterFactory)
            initializer.init()
        }
    }
